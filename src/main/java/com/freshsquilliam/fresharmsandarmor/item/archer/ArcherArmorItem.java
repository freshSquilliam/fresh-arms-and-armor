...
import com.freshsquilliam.fresharmsandarmor.client.ArcherArmorRenderer;
...
public class ArcherArmorItem extends ArmorItem implements GeoItem {
    ...
    public ArcherArmorItem(ArmorMaterial material, Type type, Properties properties) {
    ...
    public void initializeClient(Consumer<IClientItemExtensions> consumer) {
        consumer.accept(new IClientItemExtensions() {
            ...
            private ArcherArmorRenderer renderer;
...
// ===== Tooltip =====
    ...
    public void appendHoverText(
        ...
        float bonus = getDrawSpeedBonusForMaterial(this.getMaterial());

        if (bonus > 0.0F) {
            int percent = Math.round(bonus * 100);

            tooltip.add(
                    Component.literal("+" + percent + "% Bow Draw Speed")
                            .withStyle(ChatFormatting.RED)
            );
        }
    }

    private static float getDrawSpeedBonusForMaterial(ArmorMaterial material) {
